...
| **SEC-PROD-02** | Secrets / Git | **CRITICAL** | Legacy development seed passwords and test device tokens committed in repository history. | **REMEDIATED**: Removed the `admin`/`1234` seed permanently, kept development seeds disabled in production, untracked `new_device_creds.json` and `aurafind_prefs.xml`, updated `.gitignore`, and recommended key rotation before launch. |
